...
object KafkaToMongo {
  ...
  // Function to configure the session with MongoDB
  ...
  // Function to take dataFrame (Streaming) as input and writes it to the given database and collection
  ...
  def readStreamFromKafka(spark: SparkSession, topic_name: String): DataFrame = {
    // Subscribing to the Kafka-topic and reading the message stream (Source)
    //     df
    //     KEY                VALUE
